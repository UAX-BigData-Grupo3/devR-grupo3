...
***Paula*** acq content_transformer Corpus crude DataframeSource DirSource Docs findAssocs findFreqTerms findMostFreqTerms foreign getTokenizers getTransformations hpc inspect ***Cristian*** meta PCorpus PlainTextDocument plot readDataframe readDOC Reader readPDF readPlain readRCV1 readReut21578XML readTagged readXML removeNumbers removePunctuation ***Rafa*** removeSparseTerms removeWords SimpleCorpus Source stemCompletion stemDocument stopwords stripWhitespace TermDocumentMatrix termFreq TextDocument tm_combine tm_filter tm_map tm_reduce ***Diego*** tm_term_score tokenizer URISource VCorpus VectorSource weightBin WeightFunction weightSMART weightTf weightTfIdf writeCorpus XMLSource XMLTextDocument Zipf_n\_Heaps ZipSource
...
```{r}
base <- df_cancer[, c("Age","Grade","Tumor Size","Regional Node Examined","Reginol Node Positive","Survival Months","Status")]
...
k <- df_cancer %>% ggplot(aes(`Estrogen Status`, fill=Status)) + geom_bar()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
l <- df_cancer %>% ggplot(aes(`Progesterone Status`, fill=Status)) + geom_bar()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
m <- df_cancer %>% ggplot(aes(`Regional Node Examined`, fill=Status)) + geom_bar()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
n <- df_cancer %>% ggplot(aes(`Reginol Node Positive`, fill=Status)) + geom_bar()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
