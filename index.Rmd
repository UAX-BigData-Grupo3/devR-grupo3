...
Los documentos a analizar tendrán formato PDF y deben estar en el mismo directorio desde el que se ejecuta este Notebook.

```{r}
getwd()

# Los documentos (pdf) deben estar en el mismo directorio de trabajo que este notebook

# Generamos una lista con lo nombres de los ficheros pdf que están en el directorio

papers <- list.files(path="./papers",pattern ="pdf$")
papers <-lapply(papers,paste)

#Comprobamos que detecta los documentos (en este ejemplo son 4 papers científicos)
...
```{r}
#Inspeccionamos el contenido del corpus mostrando para cada documento el formato,
...
```{r}
set.seed(16)
wordcloud(names(frec), frec, min.freq=10, max.words = 30, colors=brewer.pal(8, "Dark2"))
```
